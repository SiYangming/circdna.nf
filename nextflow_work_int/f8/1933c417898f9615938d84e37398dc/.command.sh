...
echo '' | gzip > circdna_1_1.merged.fastq.gz
echo '' | gzip > circdna_1_2.merged.fastq.gz
...
echo '' > .command.env
#
nxf_eval_cmd STDOUT STDERR bash -c "cat --version 2>&1 | head -n 1 | sed 's/^.*coreutils) //; s/ .*$//'"
status=$?
if [ $status -eq 0 ]; then
