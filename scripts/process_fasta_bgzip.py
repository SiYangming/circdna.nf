...
def get_fai_path(gz_path):
    gz_path = Path(gz_path)
    fai_path = Path(str(gz_path) + ".fai")
    if fai_path.exists():
        return fai_path
    fai_path = gz_path.parent / f"{gz_path.stem}.fai"
    return fai_path

def get_gzi_path(gz_path):
    gz_path = Path(gz_path)
    gzi_path = Path(str(gz_path) + ".gzi")
    if gzi_path.exists():
        return gzi_path
    gzi_path = gz_path.with_suffix('.gzi')
    return gzi_path
...
def process_fasta_to_bgzip(fasta_path, output_path, seq_ids=None, remove_version=False, force=False):
    gz_path = Path(output_path).with_suffix('.fa.gz')
    fai_path = get_fai_path(gz_path)
    gzi_path = get_gzi_path(gz_path)

    if gz_path.exists():
        if force:
            print(f"⚠️ Output file exists, removing for rebuild: {gz_path.name}")
            os.remove(gz_path)
            if fai_path.exists():
                os.remove(fai_path)
            if gzi_path.exists():
            ...
            print(f"⚠️ Output file already exists, skipping: {gz_path.name}")
            return False, 0, 0
    ...
    bgzip_proc = subprocess.Popen(["bgzip", "-c"], stdin=subprocess.PIPE, stdout=open(gz_path, 'wb'))
    ...
    try:
        ...
        print(f"    Output: {gz_path.name}")
        print(f"    Compressed size: {get_file_size(gz_path)}")

        print(f"\n[3] Creating FASTA index with samtools faidx")
        cmd = ["samtools", "faidx", str(gz_path)]
...
def verify_gz_file(gz_path):
    ...
    if gz_size < 1024 * 1024:
        return False, f"File too small ({gz_size} bytes), likely corrupted"
...
def check_all_outputs():
    ...
    for species, info in SPECIES_MAP.items():
        ...
        is_valid, msg = verify_gz_file(gz_path)
        if is_valid:
            print(f"  ✅ {msg}")
        else:
            print(f"  ❌ {msg}")
...
def main():
    ...
    for species, info in SPECIES_MAP.items():
        ...
        gz_path = Path(output_path).with_suffix('.fa.gz')
        old_bgz_path = Path(output_path).with_suffix('.bgz')

        if args.clean and old_bgz_path.exists():
            print(f"�️ Removing old .bgz file: {old_bgz_path.name}")
            os.remove(old_bgz_path)
            fai_path = get_fai_path(old_bgz_path)
            if fai_path.exists():
                os.remove(fai_path)
