...
include { CRESIL_TRIM } from '/Users/siyangming/nextflow_nf_core/bio.nf/modules/cresil/trim/tests/../main.nf'
...
workflow {
    ...
    input = []
                ...
                input[0] = [ [ id:'test' ], // meta
                             file("${projectDir}/modules/cresil/testdata/exp_reads.fastq", checkIfExists: true) ]
                input[1] = [ [ id:'ref' ], // meta2
                             file("${projectDir}/modules/cresil/testdata/ref.mmi", checkIfExists: true) ]
                
    //----

    //run process
    CRESIL_TRIM.run(input.toArray())

    if (CRESIL_TRIM.output){

        // consumes all named output channels and stores items in a json file
        CRESIL_TRIM.out.getNames().each { name ->
            serializeChannel(name, CRESIL_TRIM.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = CRESIL_TRIM.out as List<Object>
        def i = 0
        array.each { output ->
