process CIRCLEFINDER {
    ...
    # Early check: if split file has no data, no eccDNA can be detected.
    # Reference: suda-huanglab/circlehunter best practice (touch empty output instead of error)
    ...
    # No reads with frequency==3 means no eccDNA signature (1 concordant + 2 split = 3)
    ...
    # Optimization: Use awk instead of grep -w -Ff for large file filtering
    awk 'NR==FNR{a[\$1]; next} \$4 in a' ${prefix}.split.id-freq2.txt ${split} > ${prefix}.split_freq2.txt
    # awk 'NR==FNR{a[\$1]; next} \$4 in a' ${prefix}.split.id-freq4.txt ${split} > ${prefix}.split_freq4.txt
    ...
    awk 'NR==FNR{a[\$1]; next} \$4 in a' ${prefix}.concordant.id-freq3.txt ${concordant} > ${prefix}.concordant_freq3.txt
    # awk 'NR==FNR{a[\$1]; next} \$4 in a' ${prefix}.concordant.id-freqGr3.txt ${concordant} > ${prefix}.concordant_freqGr3.txt
